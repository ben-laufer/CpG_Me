...
def writeHeader_permeth(ph_chrNum):
    ...
    with open(outPermDir + "PerMeth_" + trackName+ "_chr" + ph_chrNum + ".bed","wb") as permOutHeader:
...
def writeHeader_dss(dh_chrNum):
    with open(outDssDir + trackName+ "_chr" + dh_chrNum + ".DSS.txt","wb") as dssOutHeader:
...
def writeData_permeth(pd_chrNum):
    with open(outPermDir + "PerMeth_" + trackName + "_chr" + pd_chrNum + ".bed","ab") as permOutFile:
...
def writeData_dss(dd_chrNum):
    ...
    with open(outDssDir + trackName + "_chr" + dd_chrNum + ".DSS.txt","ab") as dssOutFile:
...
# if sample files are in different directories
    ...
    #with gzip.open(inDir + inFile, "rb") as bisInFile:

with gzip.open(inFile, "rb") as bisInFile:
